  ...
  db.query(
    ...
    (err, result) => {
      if (err) {
        ...
        db.query(
          "INSERT INTO users (uuid, username, email, pass_hash) VALUES (?)",
          [data],
          (err) => {
            if (err) {
              console.error("Something went wrong");
...
router.get(`/sendConfirmationEmail`, (req, res) => {
  if (req.session.uuid) {
    console.log(
      `User with uuid: "${req.session.uuid}" requesting a confirmation email.`
    );
    const token = randToken.generate(60);
    db.query(
      "UPDATE users SET token = (?) WHERE uuid = (?) AND verified = FALSE",
  ...
  db.query(
    ...
    (err, result) => {
      if (err) {
        ...
        db.query(
          "UPDATE users SET token = NULL WHERE uuid = (?)",
          [req.session.uuid],
          (err) => {
            if (err) {
              console.error("Something went wrong");
